...
-- | Data type used in makeCommentWidgetMod, containing modifications to
-- comment-action-related data structures.
data CommentMods = CommentMods
    { mod_earlier_closures
        :: [CommentClosing]
        -> [CommentClosing]
    , mod_earlier_retracts
        :: [CommentRetracting]
        -> [CommentRetracting]
    , mod_user_map
        :: Map UserId User
    ...
    , mod_tag_map
        ...
        -> Map TagId Tag
    , mod_watch_map
        :: Map CommentId (Set WatchedSubthread)
        -> Map CommentId (Set WatchedSubthread)
    }
