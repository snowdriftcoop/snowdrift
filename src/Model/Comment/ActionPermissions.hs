...
loggedOutCommentActionPermissions = emptyCommentActionPermissions
    { can_reply = True
    , can_watch = True }

makeLoggedOutCommentActionPermissionsMap :: MakeActionPermissionsMap
makeLoggedOutCommentActionPermissionsMap =
    return
    . M.fromList
    . map ((,loggedOutCommentActionPermissions) . entityKey)

-- | Action permissions that apply to a Project discussion, and a Project's
-- WikiPage discussion, and a Project's blog.
makeProjectCommentActionPermissionsMap
    :: Maybe (Entity User)
    -> Text
    -> CommentMods
    -> MakeActionPermissionsMap
makeProjectCommentActionPermissionsMap Nothing _ _ comments =
    makeLoggedOutCommentActionPermissionsMap comments
makeProjectCommentActionPermissionsMap
        (Just (Entity viewer_id viewer))
        project_handle
        CommentMods{..}
        comments = do
    ...
    let map2 :: (a -> b) -> (a -> c) -> [a] -> ([b],[c])
        ...
        (comment_ids, user_ids) =
            map2 entityKey (commentUser . entityVal) comments

    -- I don't like this formatting, but the commas have to nest under
    -- the (.
    (viewer_is_mod
     , user_map
     , closing_map
    ...
    let viewer_is_established = userIsEstablished viewer
        ...
        step (Entity comment_id comment) =
            let user_id = commentUser comment
                user = lookupErr
                    ("makeProjectCommentActionPermissions: user id not "
                    <> "found in map")
                    user_id
                    user_map
            in M.insert comment_id CommentActionPermissions
                ...
                , can_approve   =
                    viewer_is_mod && not (commentIsApproved comment)
                , can_claim     =
                    M.member comment_id ticket_map
                    && M.notMember comment_id claim_map
                , can_close     =
                    viewer_can_close
                    && M.notMember comment_id closing_map
                    && commentIsApproved comment
                , can_delete    =
                    viewer_id == user_id
                    && S.notMember comment_id comments_with_children
                , can_edit      = userCanEditComment viewer_id comment
                , can_establish = viewer_is_mod && userIsUnestablished user
                , can_flag      =
                    viewer_is_established
                    && viewer_id /= user_id
                    && M.notMember comment_id flag_map
                , can_reply     = commentIsApproved comment
                , can_rethread  = viewer_is_mod || viewer_id == user_id
                , can_retract   =
                    viewer_id == user_id
                    && M.notMember comment_id retracting_map
                    && commentIsApproved comment
                , can_unclaim   =
                    maybe False
                ...
                , can_unwatch   =
                    maybe False
                        (S.member comment_id . S.map watchedSubthreadRoot)
                        (M.lookup comment_id watch_map)
                }
...
makeUserCommentActionPermissionsMap
    ...
    -> CommentMods
    -> MakeActionPermissionsMap
makeUserCommentActionPermissionsMap Nothing _ _ comments =
    makeLoggedOutCommentActionPermissionsMap comments
makeUserCommentActionPermissionsMap
        (Just (Entity viewer_id viewer))
        user_id
    ...
    let viewer_is_established = userIsEstablished viewer
        ...
        step (Entity comment_id comment) =
            ...
            in
                M.insert comment_id CommentActionPermissions
                    { can_add_tag   = viewer_is_established
                    ...
                    , can_reply     = commentIsApproved comment
                    , can_rethread  =
                        viewer_id == user_id || viewer_id == author_id
                    , can_retract   =
                        viewer_id == author_id
                        && M.notMember comment_id retracting_map
                        && commentIsApproved comment
