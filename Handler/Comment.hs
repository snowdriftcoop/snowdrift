...
-- Checks the POST params for "mode" key:
-- could either be a "post" (posts the comment and returns its id)
-- or a "preview" (returns the comment tree and form, to wrap in a preview).
-- Permission checking should occur *PRIOR TO* this function.
postNewComment
    :: Maybe CommentId
    -> Entity User
    ...
    -> Handler (Either CommentId (Widget, Widget))
postNewComment
        mparent_id
        (Entity user_id user)
        discussion_id
