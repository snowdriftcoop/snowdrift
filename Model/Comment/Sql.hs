...
exprCommentClosedOrRetracted :: ExprCommentCond
exprCommentClosedOrRetracted c =
    c ^. CommentId `in_`   exprClosedCommentIds
    ||. c ^. CommentId `in_`   exprRetractedCommentIds

exprCommentOpen :: ExprCommentCond
exprCommentOpen c =
    c ^. CommentId `notIn` exprClosedCommentIds
    &&. c ^. CommentId `notIn` exprRetractedCommentIds
...
exprCommentRootPostedBy user_id c =
    (isNothing (c ^. CommentParent) &&. c ^. CommentUser ==. val user_id)
    ||. c ^. CommentId `in_` sublist
  where
    sublist =
        subList_select $
...
-- | SQL expression to filter a Comment (somewhere) on a Project based on
-- "permissions", as follows:
--    If moderator, show all.
--    If logged in, show all approved (hiding flagged), plus own comments
--        (unapproved + flagged).
--    If not logged in, show all approved (hiding flagged).
--    No matter what, hide rethreaded comments (they've essentially been
--        replaced).
-- TODO(mitchell, david): rethink this function with regards to comment
-- visibility
exprCommentProjectPermissionFilter
    :: Maybe UserId
...
exprCommentProjectPermissionFilter muser_id project_id c =
    exprCommentNotRethreaded c
    &&. exprCommentProjectPermissionFilterIncludingRethreaded
            muser_id
            project_id
            c

-- | A "special case" of the above (almost universal) permission filter,
-- for Project feeds: we *do* want to display rethreaded Comments in this
-- case, because otherwise, the original "comment posted" feed events would
-- vanish.
exprCommentProjectPermissionFilterIncludingRethreaded
    :: Maybe UserId
    -> SqlExpr (Value ProjectId)
    -> ExprCommentCond
exprCommentProjectPermissionFilterIncludingRethreaded muser_id project_id c =
    isVisible &&. permissionFilter
  where
    ...
    isVisible = case muser_id of
        Just user_id ->
            c ^. CommentVisibility ==. val VisPublic
            ||. exprUserIsTeamMember user_id project_id
            ||. exprCommentRootPostedBy user_id c
        Nothing -> c ^. CommentVisibility ==. val VisPublic

    permissionFilter = case muser_id of
        Just user_id ->
            approvedAndNotFlagged
            ||. exprCommentPostedBy user_id c
            ||. exprUserIsModerator user_id project_id
        Nothing      -> approvedAndNotFlagged

    approvedAndNotFlagged =
        exprCommentApproved c &&. not_ (exprCommentFlagged c)
...
exprCommentUserPermissionFilter mviewer_id user_id c =
    exprCommentNotRethreaded c
    &&. exprCommentUserPermissionFilterIncludingRethreaded mviewer_id user_id c

-- | A "special case" of the above (almost universal) permission filter,
-- for User feeds: we *do* want to display rethreaded Comments in this
-- case, because otherwise, the original "comment posted" feed events would
-- vanish.
exprCommentUserPermissionFilterIncludingRethreaded
    :: Maybe UserId
    -> SqlExpr (Value UserId)
    -> ExprCommentCond
exprCommentUserPermissionFilterIncludingRethreaded mviewer_id user_id c =
  ...
  where
    ...
    isVisible = case mviewer_id of
        Just viewer_id ->
            c ^. CommentVisibility ==. val VisPublic
            ||. val mviewer_id ==. just user_id
            ||. exprCommentRootPostedBy viewer_id c
        Nothing -> c ^. CommentVisibility ==. val VisPublic

    permissionFilter = case mviewer_id of
        Just viewer_id ->
            approvedAndNotFlagged ||. exprCommentPostedBy viewer_id c
        Nothing      -> approvedAndNotFlagged
